...
function validEmail() {
  let emailRegExp = new RegExp(
    "^[a-zA-Z0-9.-_]{2,99}[@]{1}[a-zA-Z0-9.-_]+[.]{1}[a-z]{2,10}$",
    "g"
  );
  ...
  if (!emailValid) {
    smallColorRed(
      inputEmail,
      "Veuillez entrer une adresse email valide (xyz@exemple.com)!"
    );
    return false;
  } else {
...
function validQuantity() {
  ...
  if (!quantityValid) {
    smallColorRed(
      inputQuantity,
      "Veuillez entrer une valeur numérique entre zéro - 99 !"
    );
    return false;
  } else {
...
function validate(e) {
  if (
    ...
    validLocalisation(inputLocalisation) &&
    validLu(inputObligatoire)
  ) {
    e.preventDefault();
